...
class neuro(nn.Module):
    ...
    def forward(self, D, S, hsd, od, os, ref):
        ...
        sim = torch.sigmoid(self.dyn(hsd, ref))
...
class RSDN9_128(nn.Module):
    def __init__(self, opt):
        ...
        self.nf = 128
        self.opt=opt
        self.neuro = neuro(self.nf, self.opt.scale)
        self.scale = self.opt.scale
        self.down = PixelUnShuffle(self.opt.scale)
        self.conv = nn.Conv2d(48*2, 48, 3, 1, 1)
        initialize_weights([self.conv],0.1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
        
    def forward(self, LR, D, S):#这个代表模型输入
        _,T,_,_,_ = D.shape
        assert ( T >= 3 )
        out = []
        out_d = []
